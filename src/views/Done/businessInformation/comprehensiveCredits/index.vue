<template>
  <div class="retail_credit">
    ...
    <div class="retail_top_div">
      ...
      <horizontal-info
        title="担保金额："
        :value="item.GUARANTYSUM"
      ></horizontal-info>
      <horizontal-info
        title="担保类型："
        :value="item.GUARANTYTYPENAME"
      ></horizontal-info>
      <horizontal-info
        title="合同编码："
        :value="item.GCSERIALNO"
      ></horizontal-info>
      <horizontal-info
        title="客户经理："
        :value="item.INPUTUSERNAME"
      ></horizontal-info>
    </div>
    <!-- 中间核保人员信息 -->
    ...
    <div class="retail_db_info">
      <div class="top_color">
        <div class="danbao_title_div">
          ...
          <span>担保人:{{ item.GUARANTORNAME }}</span>
        </div>
        <horizontal-line></horizontal-line>
        <div class="ht_div">
          <div class="ht_left">
        ...
        <div class="ht_div">
          <div class="ht_left">
            <img src="@/assets/images/sqs_pic.png" class="images" />
            <span>授权书</span>
          </div>
          <div class="ht_right" @click="toImageData('sqs')">
            <span>{{ text1 }}</span>
            <img src="@/assets/images/cx_jr_icon.png" class="img" />
          </div>
        </div>
...
export default {
  ...
  components: { ItemNavBar, HorizontalInfo, HorizontalLine },
  props: {},
  data() {
    return {
  ...
  methods: {
    ...
    getImgData() {
      ...
      ajaxPost(url, {
        inputDate: this.item.DISPOSALDATE,
        taskNo: this.item.SERIALNO,
      })
        .then((res) => {
          this.$toast.clear();
          let imgList = res.url;
