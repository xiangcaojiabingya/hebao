<template>
  ...
  <div class="content">
    <!-- 引入头部导航栏组件 -->
    <item-nav-bar title="查询"></item-nav-bar>
    <!-- 搜索栏 -->
    <div class="fixed">
  ...
  methods: {
    ...
    getContent(data) {
      ...
      if (data.name == "" && data.phone == "") {
        ...
        this.pageNum = "1";
        this.finished_text = "";
        if (this.permissions == "hbfp" || this.permissions == "hb") {
          this.getQuery();
        } else if (this.permissions == "fp") {
    ...
    getQuery() {
      ...
      var url = {};
      if (this.category == "zh") {
        url = apiUrl.taskQuery;
      } else {
        url = apiUrl.queryLshb;
      }
      ajaxPost(url, data)
        .then((res) => {
          this.$toast.clear();
    ...
    // 上滑加载
    onload() {
      if (this.queryPhone != "" || this.queryName != "") {
        if (this.permissions == "hbfp" || this.permissions == "hb") {
          this.getQuery();
        } else if (this.permissions == "fp") {
          this.getDistribution();
    ...
    // STATUS 1 待分配  2 未完成 3 已完成 4 收回  5 退回 6 作废 7 重新分配
    //  DISPOSALDATE 日期处理
    btn(list) {
      if (this.category == "zh") {
      ...
      if (
        ...
        list.CUSTOMERTYPE.slice(0, 2) == "01"
      ) {
        this.$router.push({
      ...
      // 完成/退回个人
        ...
        (list.STATUS == "3" || list.STATUS == "4" || list.STATUS == "5") &&
        list.CUSTOMERTYPE.slice(0, 2) == "03"
      ) {
        this.$router.push({
          path: "/retailCredits",
          query: {
      ...
      // 未完成对公
      else if (list.STATUS == "2" && list.CUSTOMERTYPE.slice(0, 2) == "01") {
        this.$router.push({
      ...
      // 未完成个人
      else if (list.STATUS == "2" && list.CUSTOMERTYPE.slice(0, 2) == "03") {
        this.$router.push({
          path: "/retailCredit",
          query: {
            item: JSON.stringify(list),
    ...
    getDistribution() {
      ...
      var url = {};
      if (this.category == "zh") {
        url = apiUrl.queryUser;
      } else {
        url = apiUrl.queryLsfp;
      }
      ajaxPost(url, data)
        .then((res) => {
          this.$toast.clear();
